...
def is_crossing_street(pedestrian_path, map):
    return (
        pedestrian_path.intersects(map.road_poly)
        or pedestrian_path.intersects(map.crosswalk_poly)
        or pedestrian_path.intersects(map.intersection_poly)
        or pedestrian_path.intersects(map.gap_poly)
    )


def is_illegal_crossing(pedestrian_path, map):
    return pedestrian_path.intersects(map.road_poly) or pedestrian_path.intersects(
...
def get_recorded_features(pedestrian_id, dataset, input_len=90):
    ...
    if type(dataset) is not dict:
        ...
        for _ in range(6):
            data.append(
                pd.DataFrame(dataset).iloc[pedestrian_id, :][idx : idx + input_len]
            )
            idx += input_len
        ...
        x, y, vx, vy, ax, ay = (
            dataset[pedestrian_id]["x"].to_numpy(),
            dataset[pedestrian_id]["y"].to_numpy(),
            dataset[pedestrian_id]["vx"].to_numpy(),
            dataset[pedestrian_id]["vy"].to_numpy(),
            dataset[pedestrian_id]["ax"].to_numpy(),
            dataset[pedestrian_id]["ay"].to_numpy(),
        )

    return x, y, vx, vy, ax, ay


def get_data_where_pedestrians_move(
    attributes, threshold: float = 0.5, velocity_filter: bool = True
):
...
def split_pedestrian_data(
    data, remove_staionary_data: bool = False, chunk_size: float = 90
):
    """
    ...
    for pedestrian_id, attributes in data.items():
        ...
        num_chunks = len(new_attributes["x"]) // chunk_size + (
            1 if flag_extra_data else 0
        )

        for i in range(num_chunks):
            split_data[idx] = {}
            max_size = chunk_size + (
                extra_data if ((not flag_extra_data) and (i == num_chunks - 1)) else 0
            )
...
def create_dataframe(dataset, input_len=90):
    ...
    for key in range(len(dataset)):
        ...
        proximity_to_road__min = min(
            [Point(x[i], y[i]).distance(map.road_poly) for i in range(len(x))]
        )
        ...
        proximity_to_inter__min = min(
            [Point(x[i], y[i]).distance(map.intersection_poly) for i in range(len(x))]
        )
        ...
        new_row_data["acc__median"] = acceleration.quantile(0.5)[0]
        new_row_data["acc__first_quantile"] = acceleration.quantile(0.25)[0]
        new_row_data["acc__last_quantile"] = acceleration.quantile(0.75)[0]
        ...
        new_row_data["speed__median"] = speed.quantile(0.5)[0]
        new_row_data["speed__first_quantile"] = speed.quantile(0.25)[0]
        new_row_data["speed__last_quantile"] = speed.quantile(0.75)[0]
        ...
        new_row_data["direction_change__median"] = direction_change.quantile(0.5)[0]
        new_row_data["direction_change__first_quantile"] = direction_change.quantile(
            0.25
        )[0]
        new_row_data["direction_change__last_quantile"] = speed.quantile(0.75)[0]
        ...
        new_row_data["starting_point"] = (
            check_point_location(Point(x[0], y[0])) + "__start"
        )
        new_row_data["ending_point"] = (
            check_point_location(Point(x[-1], y[-1])) + "__end"
        )
...
def create_dataframes_with_different_chunk_sizes(start=5, end=90, step=5):
    ...
    for chunk_size in range(start, end, step):
        (dataset, pedestrian_ids), data_type = (
            split_pedestrian_data(
                sind.pedestrian_data, remove_staionary_data=False, chunk_size=chunk_size
            ),
            f"__full_splitted_data_{chunk_size}",
        )

        # df = create_dataframe(dataset)
        # df.to_csv(ROOT + f"/dataset_created{data_type}.csv", encoding='utf-8', index=False)
        pedestrian_ids.to_csv(
            ROOT + f"/pedestrian_ids_{data_type}.csv", encoding="utf-8", index=False
        )
