...
class Clusters(object):
    def __init__(self, embeddings, target, padding_masks, config=None):
        ...
        self.target = target
        self.df_predicted = pd.DataFrame(embeddings.reshape(embeddings.shape[0], -1)) # flatten time dimension data - merge last two dimensions
        self.df_target = pd.DataFrame(target.reshape(target.shape[0], -1)) # flatten data merge last two dimensions
        self.padding_masks = padding_masks
        self.config = config

        self.data_pooled = np.mean(embeddings, axis=1)
        self.data_pooled_original = np.ma.masked_array(target, mask=np.broadcast_to(~padding_masks[:, :, None], target.shape)).mean(axis=1).data
    ...
    def load(self, filename):
        if self.config is not None and os.path.exists(self.config['output_dir'] + '/clusters'):
            filepath = os.path.join(self.config['output_dir'] +'/clusters', filename)
        else: 
            filepath = os.path.join(ROOT_RESOURCES, filename)
    ...
    def plot_clusters(self, clusters_df: pd.DataFrame, remove_noise=True):
        ...
        for cluster in np.unique(clusters_df):
            if (remove_noise and cluster != -1) or True:  # -1 is not assigned clusters
                ...
                map_instance.plot_dataset(pedestrian_data=trajectories, ax=ax, color=COLOR_PALETTE[cluster], title=f'Cluster {cluster}', alpha_trajectories=0.3, padding_masks=self.padding_masks)

        plt.show()
    

class HDBSCANCluster(Clusters):
    def __init__(self, embeddings, target, padding_masks, min_cluster_size=5, min_samples=30, config=None):
        super().__init__(embeddings=embeddings, target=target, padding_masks=padding_masks, config=config)
    ...
    def cluster_data(self, data: pd.DataFrame):
        self.clusterer = HDBSCAN(min_cluster_size=self.min_cluster_size, min_samples=self.min_samples)
    ...
    def get_silhouette_score(self, data: pd.DataFrame, clusters: pd.DataFrame, remove_noise: bool = True):
    ...
    def run(self, original_data: bool = False, remove_noise: bool = True, save_data: bool = True, show_clusters: bool = False):
        if original_data: data = self.data_pooled_original
        else: data = self.data_pooled

        self.cluster_data(data)
        self.get_silhouette_score(data, self.clusters, remove_noise)

        if save_data: 
            self.save_clusters(f'hdbscan_model{"_original" if original_data else ""}.pkl', self.clusterer)
            self.save_clusters(f'cluster_labels{"_original" if original_data else ""}.pkl', self.clusters)
