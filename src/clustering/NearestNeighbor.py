...
class ApproximateNearestNeighbors(object):
    ...
    def load_data(self):
        ...
        data = self.load(
            f'data_{"original"if self.original_data else "embeddings"}.pkl'
        )
        self.data = np.mean(
            data, axis=1
        )  # pool mean of the embeddings along the time dimension
        self.dimension = self.data.shape[1]
        self.clusters = self.load(
            f'cluster_labels{"_original" if self.original_data else ""}.pkl'
        )
...
class AnnoyModel(ApproximateNearestNeighbors):
    ...
    def save_index(self, filename: str = "annoy_index.ann"):
        filepath = os.path.join(
            self.file_root,
            f"original_{filename}" if self.config["original_data"] else filename,
        )
    ...
    def get(self, data: np.array):
        ...
        if data.ndim == 3 and data.shape[0] == 1:
            ...
            print("Data shape must be (1, n, m) or (n, m)")
