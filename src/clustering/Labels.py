...
def label_trajectories(data_original: np.array, padding_masks: np.array, clusters: pd.DataFrame, labels: pd.DataFrame):
...
def plot_trajectories_per_label(labeling_oracle, trajectories, padding_masked, COLOR_PALETTE=COLORS):
    for key, label in REVERSED_LABELS.items():
        labeling_oracle.map.plot_dataset(pedestrian_data=trajectories[key], color=COLOR_PALETTE[key], title=f'{label}', alpha_trajectories=.85, size_points=5, padding_masks=padding_masked[key])


def plot_trajectories_per_label_color_clusters(labeling_oracle, trajectories, padding_masked, clusters, COLOR_PALETTE=COLORS):
    key = 3
    labeling_oracle.map.plot_dataset_color_clusters(pedestrian_data=trajectories[key], colors=COLOR_PALETTE, clusters=clusters[key], title=f'{REVERSED_LABELS[key]}', alpha_trajectories=0.9, size_points=0, padding_masks=padding_masked[key])
...
def plot_clusters_distribution_per_label(labels, data_original, clusters, COLOR_PALETTE=COLORS):
    data_pool_original = pd.DataFrame(np.mean(data_original, axis=1), columns=["x", "y", "vx", "vy", "ax", "ay"])
    ...
    for i, (idx, row) in enumerate(category_proportions.iterrows()):
        plot_radar_chart(axs[i], row, row.index, f'Cluster {idx}', color=COLOR_PALETTE[i])
...
def run_labels(config: dict, remove_noise: bool = True, show_clusters: bool = False, show_labels: bool = False, show_labeled_clusters: bool = True):
    ...
    df_target = pd.DataFrame(target.reshape(target.shape[0], -1)) # flatten data merge first two dimensions
    ...
    if show_labeled_clusters: 
        ...
        plot_clusters_distribution_per_label(labels=labels, data_original=target, clusters=clusters, COLOR_PALETTE=COLOR_PALETTE)