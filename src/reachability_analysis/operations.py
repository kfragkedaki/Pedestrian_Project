...
def product(
    f1: Union[pp.zonotope, np.ndarray], f2: Union[pp.zonotope, np.ndarray]
) -> pp.zonotope:
...
def create_M_w(
    total_size: int,
    Z: pp.zonotope = zonotope(np.array([0, 0]), np.array([0.05, 0.05])),
    ...
    for i in range(Z.G.shape[1]):
        ...
        for j in tqdm(
            range(1, total_size),
            desc="Creating noise zonotope",
            disable=disable_progress_bar,
        ):
            _G_w[_id + j] = np.concatenate(
                (
                    _G_w[_id + j - 1][:, 1:],
                    _G_w[_id + j - 1][:, 0].reshape(Z.G.shape[0], 1),
                ),
                axis=1,
            )
        _id = _id + j + 1
...
def linear_map(
    L: Union[int, np.ndarray], z: pp.zonotope, from_side: str = "left"
) -> pp.zonotope:
...
def input_zonotope(
    U: List[np.ndarray], N: int = 30, gamma: str = "max"
) -> List[pp.zonotope]:
    ...
    for i in range(0, N):
        z = zonotope(
            c_z=np.array([vx_mean[i], vy_mean[i]]),
            G_z=np.array([[vx_std[i], 0], [0, vy_std[i]]]),
        )
        U_k.append(z)
    return U_k
...
def visualize(
    list_of_objects: Union[List[pp.zonotope], List[np.ndarray]],
    ax: plt.Axes = None,
    alpha: float = None,
    title: str = r"pypolycontain visualization",
    ...
    for i, p in enumerate(list_of_objects):
        if p.n > 2:
            print(
                "projection on ",
                tuple_of_projection_dimensions[0],
                " and ",
                tuple_of_projection_dimensions[1],
                "dimensions",
            )
        ...
        for i in range(len(list_of_objects)):
            circs.append(
                Line2D(
                    ...
                    marker="s",
                    markersize=8,
                    markerfacecolor=list_of_objects[i].color,
