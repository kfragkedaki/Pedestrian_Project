    ...
    if not load_data:
        ...
        for _j, _data in tqdm(data.items(), desc="Retrieving input"):
            if _j != "P2":  # Specific for 8_02_1
                ...
                num_chunks = (len(x) // input_len)  # Calculate how many full chunks can be made
                for _i in range(num_chunks):
                    start_idx = _i * input_len
                    end_idx = start_idx + input_len
                    _x, _y = np.array(x.iloc[start_idx:end_idx]), np.array(y.iloc[start_idx:end_idx])
                    _vx, _vy = np.array(vx.iloc[start_idx:end_idx]), np.array(vy.iloc[start_idx:end_idx])
                    _ax, _ay = np.array(ax.iloc[start_idx:end_idx]), np.array(ay.iloc[start_idx:end_idx])
                    _frame = _f.values[start_idx]
                    if _frame in _concat_data:  # Make sure the frame index exists in the dictionary
...
def _simulation(
        ...
        for frame in tqdm(
            ...
            for _ped_id, state in _data[frame].items():
                if _ped_id in _RA:
                    ...
                    RA_c[frame].update({_ped_id: {"zonotopes": _z_all[test_cases[f'c_{c}']], "id": c}})
                    if _baseline:
    ...
    for frame in tqdm(RA_l.keys(), desc="Calculating accuracy for " + DATASET):
        ...
        for _ped_id, state in _data[frame].items():
            if _ped_id in _RA:
                if RA_l[frame]: _z_l_all = RA_l[frame][_ped_id]["zonotopes"]
                if RA_c[frame]: _z_c_all = RA_c[frame][_ped_id]["zonotopes"]
                ...
                for k in range(0, input_len-1):
                    ...
                    try:
                        ...
                        # Baseline
                        if _baseline:
                            _inside = int(is_inside(_b[0], pos_k))
                            RA_b_acc[k] += _inside
    ...
    pickle.dump(RA_b_acc / _i_l * 100, _f_b)
